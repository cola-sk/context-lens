...
```
在任何网页划选文本，即可在右侧优雅的毛玻璃侧边栏中与 Gemini、OpenAI、Claude 或本地/自定义大模型无缝对话。
```
...
【核心功能点】
1. 划词即答：在网页上划选任何段落、词汇或句子，轻点悬浮的微标按钮或通过右键菜单，即可将上下文发送到侧边栏。
2. 跨模型支持：内置支持 Google Gemini、OpenAI、Anthropic Claude 以及 Ollama/自定义 OpenAI 兼容的本地或云端大模型，可一键切换。
3. 网页上下文智能提取：可勾选“附加完整文章上下文”，插件会自动对网页进行全文精简并与当前对话合并，让大模型完美掌握整篇文章的语境。
4. 来源追溯与增强：发送提问时自动附带当前网页的地址和标题，方便 AI 大模型在必要时建议拉取或精确定位信息。
5. 标签页完全隔离：右侧侧边栏独立在当前 Tab 生效，在多标签切换时能够自动隐藏和恢复状态，绝不干扰你其他页面的浏览。
6. 现代极简设计：采用令人惊叹的 Light Glassmorphism（浅色毛玻璃/亚克力）设计系统，完美融入现代操作系统审美。
7. 键盘快捷键：支持使用 Ctrl + Enter (或 macOS 上的 Cmd + Enter) 快速发送内容，同时保留 Enter 回车换行的原生输入体验。

【如何使用】
1. 点击右上角插件图标，或在任意网页上划选文本并点击悬浮的 "Lens" 按钮以打开右侧栏。
2. 在侧边栏的“设置”（齿轮图标）中填入您的 AI 密钥（Gemini Key、OpenAI Key 等，所有密钥和配置均 100% 本地存储）。
3. 选择您想使用的 AI 服务商及模型。
4. 划选网页文本，点击悬浮图标，然后在底部输入框与 AI 进行对话。
5. 如果需要分析整篇长文，请勾选“附加完整文章上下文”以发送更充足的背景信息。

【隐私与安全保障】
ContextLens 坚守用户隐私至上的原则：
- 我们不会收集您的任何个人信息、账号数据或浏览历史。
- 您的 API 密钥和所有对话记录均存储在您浏览器本地的本地安全存储（chrome.storage.local）中，绝不上传到任何第三方中转服务器。
- 大模型请求直接发送到官方 API 端点（或您自行配置的本地端点），中间没有任何代理，您的数据完全属于您自己。
```
...
| `scripting` | permissions | **中文：** 当用户主动勾选“附加完整文章上下文”时，用于向当前活动网页中动态注入轻量级解析脚本，提取出网页中的核心正文文本并做自动去噪精简，然后提供给侧边栏内的 AI 进行长文解读。<br>**English:** Used to dynamically execute a lightweight reader-mode script on the active page to extract and clean main article body text only when the user explicitly requests to send the full-page context. |
...
| 版本号 | 发布日期 | 更新内容 | 审核状态 |
| :--- | :--- | :--- | :--- |
| **1.0.0** | 2026-05-20 | - 首次公开发布版本<br>- 支持划词自动展示 Lens 悬浮按钮及右键菜单拉起侧边栏<br>- 支持 Gemini, OpenAI, Claude 和自定义本地大模型 (Ollama)<br>- 支持附加完整网页正文及 URL 引用功能<br>- 支持 Ctrl + Enter 快捷发送，Enter 回车折行输入<br>- 拥有精致的毛玻璃浅色设计界面 | 🟡 准备打包中 |
