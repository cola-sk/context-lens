...
- **本地编程代理**：通过 Bridge Server 对接 Claude Code、Codex CLI、Antigravity CLI、Copilot CLI，选中 UI 文本即可让本地代理定位源码并执行修改。
...
如需使用 Claude Code / Codex CLI / Antigravity CLI / Copilot CLI 本地代理功能：
...
2. 在 **Basic Config** 标签页中管理模型：
   - **本地代理**：自动检测已安装的 CLI 代理（Claude Code、Codex、Antigravity、Copilot），显示可用状态和版本。点击 **Refresh Agents** 重新探测。
